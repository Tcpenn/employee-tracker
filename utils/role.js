...
const { createDepartmentArr, createRoleArr } = require ('./general')
        ...
        db.query(sql, (err, result) => {
            ...
            inquirer.prompt([
                ...
                if (response.update === 'Name') {
                    ...
                    // the salary parameter is based off of the response in the above inquirer prompts while the role to search with
...
function deleteRole(db) {
    //sql request to return title of role
    const sql = `SELECT role.title
                    FROM role`;

    db.query(sql, (err, result) => {
        //creates a role array based on the titles
        const roleArr = createRoleArr(result);

        //adds exit to array of options
        roleArr.push('Exit');
